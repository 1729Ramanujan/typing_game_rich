    ...
    for (let i = 0; i < 4; i++) {
        ...
        movebuttons[i].onclick = () => {
            ...
            if (opp_pokemon.hp <= 0) {
                ...
                setTimeout(() => {
                    currentopp_index = Math.floor(Math.random() * opp_party.length);
                    opp_pokemon = opp_party[currentopp_index];
                    updateHP(user_pokemon, opp_pokemon);
                    $(".explanation").text("あいては" + opp_pokemon.name + "をくりだした！"); enableButtons(movebuttons);
                    document.getElementById("opppokemon").src = opp_pokemon.img;
                }, 1000);

            } else {
                setTimeout(() => {
                    opp_pokemon.moves[Math.floor(Math.random() * opp_pokemon.moves.length)].action(opp_pokemon, user_pokemon);
                    updateHP(user_pokemon, opp_pokemon);
                    if (user_pokemon.hp <= 0) {
                        ...
                        if (user_party.length === 0) {
                            setTimeout(() => {
                                $(".explanation").text("たたかえるポケモンはもういない...");
                            }, 1000);
                            setTimeout(() => {
                                showscreen(start);
                            }, 2000);
                        ...
                        setTimeout(() => {
                            currentuser_index = Math.floor(Math.random() * user_party.length);
